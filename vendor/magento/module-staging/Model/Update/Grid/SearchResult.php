...
class SearchResult extends AbstractSearchResult
{
    ...
    protected function prepareData()
    {
        ...
        foreach ($this->_items as $id => $item) {
            ...
            $itemStartTime = $item->getStartTime() ? strtotime($item->getStartTime()) : 0;

            if (count($includes) === 0 && $itemStartTime < $currentDate->getTimestamp()) {
                unset($this->_items[$id]);
                continue;
            }
    ...
    /**
     * Retrieve the total count of the search result.
     *
     * @return int
     */
